...
PhysicsObject::PhysicsObject(rp3d::PhysicsWorld *world, const glm::vec3 &position, const glm::quat &rotation,
                             float mass)
    : m_world(world), m_body(nullptr), m_collider(nullptr), m_position(position), m_rotation(rotation), m_mass(mass),
      m_restitution(0.5f), m_friction(0.3f), m_isStatic(mass == 0.0f)
{}
...
void PhysicsObject::initialize()
{
    ...
    if (shape)
    {
        // Add collider to body with identity transform (collider is in body's local space)
    ...
    // Set body type and configure mass AFTER adding collider
    if (m_isStatic)
    {
    ...
    else
    {
        ...
        // CRITICAL: For dynamic bodies, we must properly set up mass properties
        if (m_mass > 0.0f)
        {
            // Update mass properties from colliders
            m_body->updateMassPropertiesFromColliders();

            // Then set the mass (this will scale the inertia tensor appropriately)
            m_body->setMass(m_mass);

            // Update the local inertia tensor
            m_body->updateLocalInertiaTensorFromColliders();
        }

        // Add damping to stabilize corner collisions
        m_body->setLinearDamping(0.05f); // Small linear damping
        m_body->setAngularDamping(0.1f); // Slightly more angular damping

        // Enable gravity and ensure body is active
