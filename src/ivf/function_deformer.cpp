...
FunctionDeformer::FunctionDeformer() 
    : m_scale(1.0f), m_offset(0.0f), m_useLocalSpace(true), m_time(0.0f) {}

FunctionDeformer::FunctionDeformer(const DisplacementFunction& func)
    : m_displacementFunction(func), m_scale(1.0f), m_offset(0.0f), 
      m_useLocalSpace(true), m_time(0.0f) {}
...
FunctionDeformer::DisplacementFunction FunctionDeformer::createSineWave(
    float frequency, float amplitude, const glm::vec3& direction) {
...
FunctionDeformer::DisplacementFunction FunctionDeformer::createRipple(
    float frequency, float amplitude, const glm::vec3& center) {
    ...
    return [frequency, amplitude, center](const glm::vec3& pos) -> glm::vec3 {
        ...
        if (distance < 0.001f) return glm::vec3(0.0f);
...
FunctionDeformer::DisplacementFunction FunctionDeformer::createTurbulence(
    float scale, float intensity, int octaves) {
...
FunctionDeformer::DisplacementFunction FunctionDeformer::createSpiral(float pitch, float radius) {
    return [pitch, radius](const glm::vec3& pos) -> glm::vec3 {
        ...
        glm::vec3 spiralOffset(
            std::cos(angle) * spiralRadius - pos.x,
            0.0f,
            std::sin(angle) * spiralRadius - pos.z
        );
...
FunctionDeformer::DisplacementFunction FunctionDeformer::createTwirl(
    const glm::vec3& center, float strength, float falloff) {
...
FunctionDeformer::DisplacementFunction FunctionDeformer::createGravityWell(
    const glm::vec3& center, float strength, float radius) {
    
    return [center, strength, radius](const glm::vec3& pos) -> glm::vec3 {
        glm::vec3 toCenter = center - pos;
        float distance = glm::length(toCenter);
        
        if (distance < 0.001f || distance > radius) return glm::vec3(0.0f);
