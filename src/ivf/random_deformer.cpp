...
RandomDeformer::RandomDeformer(NoiseType noiseType, DeformationMode mode)
    : m_noiseType(noiseType), m_mode(mode), m_intensity(1.0f), m_frequency(1.0f), m_offset(0.0f), m_direction(0, 1, 0),
      m_center(0.0f), m_time(0.0f), m_seed(12345), m_octaves(4), m_persistence(0.5f), m_lacunarity(2.0f), m_rng(m_seed),
      m_uniformDist(-1.0f, 1.0f), m_gaussianDist(0.0f, 1.0f), m_cacheValid(false)
{}
...
glm::vec3 RandomDeformer::generateNoise(const glm::vec3 &position, GLuint vertexIndex) const
{
    ...
    case NoiseType::UNIFORM:
        ...
        noise = glm::vec3(m_uniformDist(m_rng), m_uniformDist(m_rng), m_uniformDist(m_rng)) * m_intensity;
        break;

    case NoiseType::GAUSSIAN:
        m_rng.seed(m_seed + vertexIndex);
        noise = glm::vec3(m_gaussianDist(m_rng), m_gaussianDist(m_rng), m_gaussianDist(m_rng)) * m_intensity;
        break;
    ...
    for (int dx = -1; dx <= 1; ++dx)
    {
        for (int dy = -1; dy <= 1; ++dy)
        {
            for (int dz = -1; dz <= 1; ++dz)
            {
                ...
                unsigned int hash =
                    ((cellX * 73856093) ^ (cellY * 19349663) ^ (cellZ * 83492791) ^ m_seed) & 0x7fffffff;
...
void ivf::RandomDeformer::setupProperties()
{
    Deformer::setupProperties();
    // this->addProperty("Noise Type", &m_noiseType, "RandomNoise");
    // this->addProperty("Deformation Mode", &m_mode, "Deformation Mode", "Mode of deformation to apply");
    this->addProperty("Intensity", &m_intensity, "RandomNoise");
    this->addProperty("Frequency", &m_frequency, "RandomNoise");
    this->addProperty("Offset", &m_offset, "RandomNoise");
    this->addProperty("Direction", &m_direction, "RandomNoise");
    this->addProperty("Center", &m_center, "RandomNoise");
    this->addProperty("Time", &m_time, "RandomNoise");
    // this->addProperty("Seed", &m_seed, "RandomNoise");
    this->addProperty("Octaves", &m_octaves, "RandomNoise");
    this->addProperty("Persistence", &m_persistence, "RandomNoise");
    this->addProperty("Lacunarity", &m_lacunarity, "RandomNoise");
}
