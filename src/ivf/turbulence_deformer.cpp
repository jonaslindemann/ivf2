...
TurbulenceDeformer::TurbulenceDeformer(float scale, float intensity, int octaves,
                                       float persistence, float animationSpeed)
    : m_scale(scale), m_intensity(intensity), m_octaves(octaves),
      m_persistence(persistence), m_animationSpeed(animationSpeed), m_seed(12345) {
    updateFunction();
}
...
std::unique_ptr<Deformer> TurbulenceDeformer::clone() const {
    auto cloned = std::make_unique<TurbulenceDeformer>(m_scale, m_intensity, m_octaves, m_persistence, m_animationSpeed);
...
float TurbulenceDeformer::turbulence(const glm::vec3& position, int octaves, float persistence, unsigned int seed) {
    ...
    for (int i = 0; i < octaves; ++i) {
        ...
        unsigned int hash = ((int(p.x) * 73856093) ^ (int(p.y) * 19349663) ^ 
                           (int(p.z) * 83492791) ^ seed) & 0x7fffffff;
